...
const Customers: React.FC = () => {
    ...
    const [customerStats, setCustomerStats] = useState<Record<number, { visits: number; totalSpent: number }>>({});
    ...
    const loadCustomerStats = async () => {
        try {
            ...
            const orders = Array.isArray(response.data) ? response.data : (response.data?.data || response.data || []);
            const stats: Record<number, { visits: number; totalSpent: number }> = {};

            orders.forEach((order: any) => {
                if (order.customer_id) {
                    if (!stats[order.customer_id]) {
                        stats[order.customer_id] = { visits: 0, totalSpent: 0 };
                    }
                    stats[order.customer_id].visits += 1;
                    stats[order.customer_id].totalSpent += (order.net_amount || 0);
                }
            });
                    ...
                    <TableBody>
                        {loading ? (
                            ...
                            filteredCustomers.map((customer) => {
                                const stats = customerStats[customer.id] || { visits: 0, totalSpent: 0 };
                                return (
                                    <TableRow key={customer.id} hover>
                                        ...
                                        <TableCell><Typography fontWeight={600}>{stats.visits}</Typography></TableCell>
                                        <TableCell><Typography fontWeight={700} color="#FF8C00">रू {stats.totalSpent.toLocaleString()}</Typography></TableCell>
