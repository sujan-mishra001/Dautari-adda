...
const OrderTaking: React.FC = () => {
    ...
    const handlePlaceOrder = async () => {
        ...
        try {
            ...
            // For simplicity, we create KOT/BOT for all current items if it's a new order
            // Or only if we track which items are new.
            // Requirement says "pop pop pop", let's just create KOT/BOT for the whole order 
            // but normally we should only send NEW items. 
            // I'll filter items that don't have an 'id' yet or where quantity changed.
            // But mapped existing items DO have an id. Let's send only new items for KOT.
            ...
            alert(existingOrder ? "Order updated successfully!" : "Order placed successfully!");
            navigate('/pos');
        } catch (error: any) {
            ...
            <Box sx={{
                        ...
                        orderItems.map((item) => (
                            ...
                            >
                                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                                    ...
                                    <Typography fontWeight={800} color="#1e293b">Rs. {item.price * item.quantity}</Typography>
                                </Box>
                ...
                <Box sx={{ p: 3, bgcolor: '#f8fafc', borderTop: '1px solid #f1f5f9' }}>
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
                        ...
                        <Typography fontWeight={700}>Rs. {Math.round(total * 0.05)}</Typography>
                    </Box>
                    <Divider sx={{ mb: 2, borderStyle: 'dashed' }} />
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 3 }}>
                        <Typography variant="h6" fontWeight={800}>Total Payable</Typography>
                        <Typography variant="h6" fontWeight={800} color="#FF8C00">Rs. {Math.round(total * 1.05)}</Typography>
                    </Box>
