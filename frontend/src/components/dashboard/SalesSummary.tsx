            ...
            {hasData ? (
                <Grid container spacing={2} sx={{ alignItems: 'center' }}>
                    <Grid size={{ xs: 7 }}>
                            ...
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mt: 0.5 }}>
                                ...
                                {discount > 0 && (
                                    <Typography variant="caption" color="#94a3b8" sx={{ ml: 1 }}>
                                        (Gross: रू {grossSales.toLocaleString()})
                                    </Typography>
                                )}
                            </Box>
                        </Box>
                        <Box sx={{ mt: 3 }}>
                            {[
                                { label: 'Paid Sales', val: `रू ${paidSales.toLocaleString()}`, percent: paidPercent, color: '#22c55e', icon: <TrendingUp size={12} /> },
                                { label: 'Credit Sales', val: `रू ${creditSales.toLocaleString()}`, percent: creditPercent, color: '#ff9800', icon: <TrendingDown size={12} /> },
                                { label: 'Discount Given', val: `रू ${discount.toLocaleString()}`, percent: discountPercent, color: '#ef4444', icon: null },
                            ].map((item) => (
